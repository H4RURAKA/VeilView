...
- 뒤로 / 앞으로 / 새로고침 / URL 이동
- 우클릭 드래그 기반 마우스 제스처
- 제스처별 동작 설정: 이전/다음 페이지, 왼쪽/오른쪽 탭 이동, 탭 닫기, 새로고침
- 항상 위 표시 토글
- 투명도 버튼 `0%`, `30%`, `70%`
...
VeilView는 브라우저 화면 위에서 **마우스 오른쪽 버튼을 누른 채 드래그**하는 방식의 제스처를 지원합니다.
오른쪽 버튼을 누른 상태로 움직인 뒤 버튼을 떼면, 인식된 제스처에 연결된 동작이 실행됩니다.
...
| --- | --- | --- |
| `←` | 이전 페이지 | 현재 탭에서 뒤로 이동 |
| `→` | 다음 페이지 | 현재 탭에서 앞으로 이동 |
| `↕` | 새로고침 | 위아래 또는 아래위로 드래그 |
| `┌` | 왼쪽 탭 이동 | 첫 번째 탭에서 실행하면 마지막 탭으로 이동 |
| `┐` | 오른쪽 탭 이동 | 마지막 탭에서 실행하면 첫 번째 탭으로 이동 |
| `┘` | 탭 닫기 | 현재 탭 닫기 |
| `└` | 동작 없음 | 설정에서 원하는 동작으로 변경 가능 |

상단의 `제스처` 버튼을 누르면 각 제스처에 연결할 동작을 바꿀 수 있습니다. 선택 가능한 동작은 다음과 같습니다.
...
오른쪽 클릭만 짧게 누르는 경우에는 기존 웹페이지 컨텍스트 메뉴를 최대한 유지하고, 실제 드래그로 인식된 경우에만 제스처가 실행됩니다.
...
`VeilView_v0.3.1` 같은 상위 폴더를 한 번 더 감싸서 올리지 말고, 이 저장소의 파일들이 GitHub 저장소 루트에 바로 보이도록 올리는 것을 권장합니다.

## GitHub Actions

- `.github/workflows/build.yml`: push 또는 pull request마다 Windows x64 단일 실행 파일을 빌드하고 artifact로 업로드합니다.
- `.github/workflows/release.yml`: `v0.3.1` 같은 태그를 푸시하면 GitHub Release를 만들고 `VeilView.exe`와 압축 파일을 업로드합니다.
